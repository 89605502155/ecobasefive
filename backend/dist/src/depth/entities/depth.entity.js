"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.Depth = void 0;
const graphql_1 = require("@nestjs/graphql");
const station_entity_1 = require("../../station/entities/station.entity");
const typeorm_1 = require("typeorm");
let Depth = class Depth {
};
__decorate([
    (0, graphql_1.Field)(() => graphql_1.ID),
    (0, typeorm_1.PrimaryGeneratedColumn)({ name: 'id' }),
    __metadata("design:type", Number)
], Depth.prototype, "id", void 0);
__decorate([
    (0, graphql_1.Field)(),
    (0, typeorm_1.Column)({ name: 'depth' }),
    __metadata("design:type", Number)
], Depth.prototype, "depth", void 0);
__decorate([
    (0, graphql_1.Field)({ nullable: true }),
    (0, typeorm_1.Column)({ nullable: true, name: 'salinity' }),
    __metadata("design:type", Number)
], Depth.prototype, "salinity", void 0);
__decorate([
    (0, graphql_1.Field)({ nullable: true }),
    (0, typeorm_1.Column)({ nullable: true, name: 'DOC' }),
    __metadata("design:type", Number)
], Depth.prototype, "DOC", void 0);
__decorate([
    (0, graphql_1.Field)(() => station_entity_1.Station),
    (0, typeorm_1.ManyToOne)(() => station_entity_1.Station, (station) => station.depths),
    __metadata("design:type", station_entity_1.Station)
], Depth.prototype, "station", void 0);
Depth = __decorate([
    (0, graphql_1.ObjectType)(),
    (0, typeorm_1.Entity)({ name: 'depth' })
], Depth);
exports.Depth = Depth;
//# sourceMappingURL=depth.entity.js.map